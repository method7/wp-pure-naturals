...
if (!class_exists('CWG_Trigger_Instock_Mail')) {
    ...
    class CWG_Trigger_Instock_Mail {
        ...
        public function get_subject() {
            return apply_filters('cwginstock_subject', $this->format_data(do_shortcode($this->replace_shortcode($this->get_subject))), $this->subscriber_id);
        }

        public function get_message() {
            return apply_filters('cwginstock_message', do_shortcode($this->replace_shortcode($this->get_message)), $this->subscriber_id);
        }
        ...
        public function send() {
            ...
            //commented code not needed anymore as those details already prefilled by woocommerce core function

            $mailer = WC()->mailer();

            $sendmail = $mailer->send($to, $this->get_subject(), $this->format_html_message());
            do_action('cwg_instock_after_instock_mail', $to, $this->subscriber_id);

            if ($sendmail) {
                do_action('cwg_instock_mail_sent_success', $this->subscriber_id);
                return true;
            } else {
                do_action('cwg_instock_mail_sent_failure', $this->subscriber_id);
                return false;
            }
        }
