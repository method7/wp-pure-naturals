...
if ( !class_exists( 'IMP\Register_Plugin_Term_Meta' ) ) :
    /**
     * Term option builder
     *
	 ...
	 *
	 */
    class Register_Plugin_Term_Meta{
				...
				foreach ($this->parts as $key => $part) :
					...
					if( !empty($this->term_options[ $key ]) ) :
						...
						foreach ($this->term_options[ $key ]['fileds'] as $key => $filed) :
							...
							$ThemeTerm[$name] 	   = !empty( $get_term_options_value[$name] ) ? $get_term_options_value[$name] :  $filed['default'];
							$ThemeTermOption[$name] = !empty( $get_term_options_value[$name] ) ? $get_term_options_value[$name] :  $filed['default'];
    	...
    	**/
    	static function text( $name = '', $value = '', $label = '', $desc = '', $attr = array() )
		{
    		...
    		<<?php echo $tag ?> class="form-field term-<?php echo esc_attr( $name ) ?>-wrap">
    			<<?php echo $tag2 ?> scope="row">
    				<label for="<?php echo $name ?>"><?php echo esc_attr( $label ); ?></label>
    			</<?php echo $tag2 ?>>
    			<<?php echo $tag1 ?>>
						<?php echo Form::input( $name, $value, $attr ) ?>
    	 ...
    	 * @return html
    	**/
    static function textarea( $name = '', $value = '', $label = '', $desc = '', $attr = array() )
		{
    		...
    		<<?php echo $tag ?> class="form-field term-<?php echo esc_attr( $name ) ?>-wrap">
    			...
    			<<?php echo $tag1 ?>>
						<?php echo Form::textarea( $name, $value, $attr ) ?>
    	      <?php echo self::desc($desc) ?>
    			</<?php echo $tag1 ?>>
    		</<?php echo $tag ?>>
	    ...
	    static function select( $name = '', $value = '', $choices = array(), $label = '', $desc = '', $attr= array())
			{
	    			//change html tags ..of action edit or add
	    			$action = !empty($_GET['action']) ? $_GET['action'] : '';
	    			$tag    = ($action=="edit") ? 'div' : 'tr';
	    			$tag1   = ($action=="edit") ? 'div' : 'td';
	    			$tag2   = ($action=="edit") ? 'div' : 'th';  ?>
		 ...
		 * @return html
		 **/
		static function wp_media( $name = '', $value = '', $label = '', $desc = '', $attr = array() )
		{
    		...
    		<<?php echo $tag ?> class="form-field term-<?php echo esc_attr( $name ) ?>-wrap">
    			...
    			<<?php echo $tag1 ?>>
					<?php echo Form::wp_image($name, $value, $attr); ?>
    	        	<?php echo self::desc($desc) ?>
    			</<?php echo $tag1 ?>>
    		</<?php echo $tag ?>>
