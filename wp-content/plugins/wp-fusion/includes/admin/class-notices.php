...
class WPF_Admin_Notices {
	...
	public function plugin_activation() {
		...
		if ( wp_fusion()->settings->get( 'connection_configured' ) == false && $screen->id != 'settings_page_wpf-settings' ) {
			...
			$out .= '<p>';
			$out .=  sprintf( __( 'To finish setting up WP Fusion, please go to the %1$sWP Fusion settings page%2$s</a>.', 'wp-fusion' ), '<a href="' . get_admin_url() . '/options-general.php?page=wpf-settings#setup">', '</a>' );
			$out .= '</p>';
			$out .= '</div>';
